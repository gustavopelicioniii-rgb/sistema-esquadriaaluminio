...
CREATE POLICY "Users manage own agenda" ON public.agenda FOR ALL TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Users manage own configuracoes" ON public.configuracoes FOR ALL TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Users manage own produtos" ON public.produtos FOR ALL TO authenticated USING (auth.uid() = user_id);
