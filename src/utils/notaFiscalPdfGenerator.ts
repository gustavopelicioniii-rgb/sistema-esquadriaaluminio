...
export async function generateNotaFiscalPDF(data: NotaFiscalData) {
  ...
  pdf.setFont("helvetica", "bold");
  pdf.setTextColor(...BLACK);
  pdf.text("DACTE - Documento Auxiliar do Conhecimento de Transporte Eletrônico", rightX + 2, y + 4);
  ...
  const subVals = [data.modelo, data.serie, data.numero || "-", data.folha, `${dataEmissao} ${horaEmissao}`];
  ...
  drawCell(pdf, m + cw * 0.35, y, cw * 0.65, 10, "CFOP - NATUREZA DA PRESTAÇÃO", data.naturezaPrestacao);
  y += 10;

  // ===== ORIGEM | DESTINO =====
  const halfW = cw / 2;
  drawCell(pdf, m, y, halfW, 12, "ORIGEM DA PRESTAÇÃO", `${data.origemCidade}${data.origemCidade ? " - " : ""}${data.origemUF}`);
  drawCell(pdf, m + halfW, y, halfW, 12, "DESTINO DA PRESTAÇÃO", `${data.destinoCidade}${data.destinoCidade ? " - " : ""}${data.destinoUF}`);
  y += 12;
