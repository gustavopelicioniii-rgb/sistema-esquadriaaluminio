...
export async function generateProfessionalBudgetPDF(
  ...
  safeText(
    config.empresa?.nome || 'ALUMÍNIO ESQUADRIAS DE ALUMÍNIO LTDA',
    ML,
    y + HEADER_LOGO_AREA_H + 4
  );
  ...
  drawInfField(
    'Data para início',
    formatDateBR().split('  ')[0],
    infFieldStartX + infFieldSpacing * 2
  );
  ...
  // ─── BUILD ITEMS ARRAY ───
  const items =
    config.itensMultiplos && config.itensMultiplos.length > 0
