...
const baseWindSpeed: Record<string, number> = {
  "AC": 30, "AL": 35, "AP": 30, "AM": 28, "BA": 40, "CE": 35,
  "DF": 40, "ES": 38, "GO": 40, "MA": 32, "MT": 35, "MS": 35,
  "MG": 38, "PA": 32, "PB": 35, "PR": 42, "PE": 35, "PI": 35,
  "RJ": 40, "RN": 35, "RS": 45, "RO": 28, "RR": 28, "SC": 42,
  "SP": 40, "SE": 35, "TO": 32,
};

// Terrain Factor S1 (NBR 6123 Table 2)
const terrainFactorS1: Record<string, Record<string, number>> = {
  "I": { below75: 1.06, above75: 1.00 },
  "II": { below75: 1.00, above75: 0.94 },
...
export function calculateWindLoad(inputs: NBRInputs): WindLoadResult {
  ...
  const aspectRatio = Math.max(inputs.buildingWidth, inputs.buildingLength) / 
  ...
  if (!passed) {
    warnings.push(`Pressão requerida (${requiredResistance.toFixed(1)} Pa) excede resistência do ensaio.`);
  }
...
export function getClassificationLabel(classification: "L" | "M" | "H"): string {
  switch (classification) {
    case "L": return "Baixa (L)";
    case "M": return "Média (M)";
    case "H": return "Alta (H)";
  }
}
...
export function getTerrainDescription(category: "I" | "II" | "III" | "IV"): string {
  switch (category) {
    case "I": return "Mar calmo, lagos, zona rural plana";
    case "II": return "Terrenos com poucas árvores, cercas";
    case "III": return "Suburbano, Industrial, muitas árvores";
    case "IV": return "Centros de grandes cidades, portos";
  }
}
