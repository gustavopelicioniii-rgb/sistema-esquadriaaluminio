...
export async function generateProposalPDF(config: ProposalConfig): Promise<jsPDF> {
  ...
  const ACCENT = [0, 100, 180] as const;
  ...
  safeText(`Emitido por: ${config.vendedor || "ADMIN"}`, titleBoxX + titleBoxW/2, 20, { align: "center" });
  ...
  safeText(`${config.empresa?.endereco || ""} ${config.empresa?.telefone || ""}`.trim(), ML, footerY + 5);
