...
export async function generateProfessionalPdf(config: ProfessionalPdfConfig): Promise<void> {
  ...
  pdf.setTextColor(...BLACK);
  pdf.setFont("helvetica", "normal");
  safeText(`${config.clienteEndereco || "-"} ${config.clienteBairro || ""} ${config.clienteCidade || ""}`.trim(), infoCol1 + 22, infoY);
  ...
  if (branding.bankInfo) {
    ...
    safeText(`Banco: ${bank}  |  Agência: ${agency}  |  Conta: ${account}${ccpj ? `  |  CNPJ: ${ccpj}` : ""}`, ML + 3, y + 11);
    y += 20;
  }
  ...
  if (branding.address) {
    safeText(`${branding.address} ${branding.city ? `, ${branding.city}` : ""} ${branding.cep ? `CEP ${formatCep(branding.cep)}` : ""}`, W / 2, footerY + 4, { align: "center" });
  }
