          ...
          <div className="space-y-2">
            ...
            <div className="h-2 bg-muted rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${Math.min(100, (currentEtapaIndex / (etapasPedido.length - 1)) * 100)}%`,
                }}
              />
            </div>
            <p className="text-center text-sm font-medium">
              {etapasPedido[currentEtapaIndex]?.label || 'Etapa atual'} —{' '}
              {Math.round((currentEtapaIndex / (etapasPedido.length - 1)) * 100)}%
            </p>
          </div>
        )}
      </CardContent>
