...
function DimensionAnnotations({ dims, vb }: { dims: { w: number; h: number }; vb: number }) {
  ...
  return (
    <g opacity={0.7}>
      ...
      <line x1={4} y1={vb - 2 - tickLen} x2={4} y2={vb - 2 + tickLen} stroke={cotaColor} strokeWidth={0.5} />
      <line x1={vb - 4} y1={vb - 2 - tickLen} x2={vb - 4} y2={vb - 2 + tickLen} stroke={cotaColor} strokeWidth={0.5} />
      <text x={vb / 2} y={vb + 1} textAnchor="middle" fontSize={fontSize} fill={cotaColor} fontFamily="Inter, system-ui" fontWeight={600}>
        {dims.w}
      </text>

      {/* Height dimension — right */}
      <line x1={vb - 1} y1={3} x2={vb - 1} y2={vb - 6} stroke={cotaColor} strokeWidth={0.5} />
      <line x1={vb - 1 - tickLen} y1={3} x2={vb - 1 + tickLen} y2={3} stroke={cotaColor} strokeWidth={0.5} />
      <line x1={vb - 1 - tickLen} y1={vb - 6} x2={vb - 1 + tickLen} y2={vb - 6} stroke={cotaColor} strokeWidth={0.5} />
      <text x={vb + 1} y={(vb - 3) / 2 + 1} textAnchor="start" fontSize={fontSize} fill={cotaColor} fontFamily="Inter, system-ui" fontWeight={600}
 ...
 */
function CrossSection({ code, name, type, color, weight_per_meter, size = 64 }: ProfileSection & { color: AluminumColor; size?: number }) {
  ...
  return (
    <div className="flex flex-col items-center gap-0.5">
      ...
      <span className="text-[9px] text-muted-foreground leading-tight text-center max-w-[80px] truncate" title={`${code} — ${name}`}>
        {code}
      </span>
      {dims && (
