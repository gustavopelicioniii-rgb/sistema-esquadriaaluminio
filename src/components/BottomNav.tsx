    ...
    <>
      {moreOpen && (
        <div className="fixed inset-0 z-40 sm:hidden" onClick={() => setMoreOpen(false)}>
          <div
            className="absolute bottom-16 left-0 right-0 bg-card/98 backdrop-blur-lg border-t border-border/60 rounded-t-2xl shadow-2xl animate-in slide-in-from-bottom-4 duration-200"
      ...
      <nav
        className="fixed bottom-0 left-0 right-0 z-50 border-t bg-card/95 backdrop-blur-md sm:hidden"
        style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}
      >
        <div className="flex items-center justify-around h-16">
              ...
              >
                <div
                  className={cn(
                    ...
                    active &&
                      'after:absolute after:-bottom-1 after:h-0.5 after:w-4 after:rounded-full after:bg-primary'
                  )}
                >
                  <item.icon
                    className={cn('h-5 w-5 transition-all duration-200', active && 'text-primary')}
                  />
                </div>
                <span
                  className={cn(
          ...
          >
            <LayoutGrid
              className={cn('h-5 w-5 transition-all duration-200', moreOpen && 'text-primary')}
            />
