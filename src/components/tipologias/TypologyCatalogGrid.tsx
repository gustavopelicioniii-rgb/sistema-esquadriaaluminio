...
export function TypologyCatalogGrid({
  ...
  const handleExportPdf = () => {
    ...
    const rows = items.map(t => [
      ...
      t.subcategory
        ? SUBCATEGORIES.find(s => s.value === t.subcategory)?.label || t.subcategory
        : '–',
      String(t.num_folhas),
    ...
    <div className="flex-1 min-w-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {items.map(t => (
          <Card
            key={t.id}
            className="group hover:shadow-md transition-shadow border-border/60 overflow-hidden"
          >
            <div
            ...
            <CardContent className="p-2.5 space-y-1.5">
              ...
              <p className="text-[10px] text-muted-foreground font-mono uppercase tracking-wide truncate">
                {t.id}
              </p>
