...
export function ComponentRulesManager({ typology }: Props) {
  const { rules, loading, addRule, updateRule, deleteRule, inheritFromBase } = useCustomComponentRules(typology.id);
  ...
  return (
    <Card className="border-border/50">
              ...
              <TableBody>
                {rules.map((rule) => (
                  <TableRow key={rule.id}>
                    ...
                    <TableCell className="text-xs text-muted-foreground font-mono">{rule.component_code || "—"}</TableCell>
                    <TableCell><Badge variant="secondary" className="text-[10px]">{getTypeLabel(rule.component_type)}</Badge></TableCell>
                    <TableCell className="text-center text-xs font-mono">{rule.quantity_formula}</TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" onClick={() => handleDelete(rule.id)}>
      ...
      {/* Add/Edit Dialog */}
      <Dialog open={showForm} onOpenChange={(o) => { if (!o) { setShowForm(false); setEditId(null); setForm(emptyRule); } }}>
          ...
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Input value={form.component_name} onChange={(e) => setForm({ ...form, component_name: e.target.value })} placeholder="Ex: Roldana c/ regulagem" />
              </div>
              <div className="space-y-2">
                <Label className="text-xs">Código</Label>
                <Input value={form.component_code} onChange={(e) => setForm({ ...form, component_code: e.target.value })} placeholder="Ex: ROL-426" />
              </div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="space-y-2">
                ...
                <Input value={form.quantity_formula} onChange={(e) => setForm({ ...form, quantity_formula: e.target.value })} placeholder="Ex: 4" />
              </div>
              <div className="space-y-2">
