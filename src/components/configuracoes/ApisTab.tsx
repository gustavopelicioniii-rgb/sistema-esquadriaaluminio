...
import {
  Plus, Trash2, Plug, Copy, Eye, EyeOff, Edit2, Save, Shield,
...
export function ApisTab({ apis, setApis }: ApisTabProps) {
  ...
  const testApiKey = async (key: string): Promise<boolean> => {
    ...
    if (looksValid) {
      ...
      setKeyTestResult("error");
      toast.error("Chave suspeita", { description: "O formato da chave não corresponde aos padrões conhecidos." });
      return false;
    }
  };
  ...
  const addApi = async () => {
    ...
    const { data, error } = await supabase.from("api_integracoes").insert({
      nome: newApi.nome, chave: newApi.chave, descricao: newApi.descricao,
    }).select().single();
  ...
  const updateApi = async () => {
    ...
    const { error } = await supabase.from("api_integracoes").update({
      nome: editingApi.nome, chave: editingApi.chave, descricao: editingApi.descricao,
    }).eq("id", editingApi.id);
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant={api.ativa ? "default" : "secondary"} className="text-[10px]">{api.ativa ? "Ativa" : "Inativa"}</Badge>
                  </div>
