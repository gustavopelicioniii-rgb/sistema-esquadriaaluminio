...
interface EmpresaTabProps {
  ...
  onFolgaChange: (
    key: 'perfil_offset' | 'vidro_largura_offset' | 'vidro_altura_offset',
    value: number
  ) => void;
  onResetFolgas: () => void;
  onSave: () => Promise<void>;
...
export function EmpresaTab({
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    await supabase
      .from('configuracoes')
      .upsert({ chave: 'logo_url', valor: logoUrl }, { onConflict: 'chave' });
    setLogoUploading(false);
  ...
  const handleRemoveLogo = async () => {
    ...
    await supabase
      .from('configuracoes')
      .upsert({ chave: 'logo_url', valor: '' }, { onConflict: 'chave' });
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input value={config.nome} onChange={e => onConfigChange('nome', e.target.value)} />
            </div>
            <div className="space-y-2">
              <Label>CNPJ</Label>
              <Input value={config.cnpj} onChange={e => onConfigChange('cnpj', e.target.value)} />
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                value={config.cidade || ''}
                onChange={e => onConfigChange('cidade', e.target.value)}
              />
            </div>
            <div className="space-y-2">
              <Label>Estado</Label>
              <Input
                value={config.estado || ''}
                onChange={e => onConfigChange('estado', e.target.value)}
              />
            </div>
          </div>
        </CardContent>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="space-y-2">
              <p className="text-sm text-muted-foreground">
                Imagem usada em orçamentos, notas fiscais e relatórios.
              </p>
              <div>
                <input
                  type="file"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                type="number"
                value={config.margem}
                onChange={e => onConfigChange('margem', e.target.value)}
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                type="number"
                value={config.descontoMax}
                onChange={e => onConfigChange('descontoMax', e.target.value)}
              />
            </div>
          </div>
        </CardContent>
      ...
      <Card className="shadow-sm border-border/50">
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={e => onFolgaChange('perfil_offset', Number(e.target.value))}
              />
            </div>
          </div>
          <Separator />
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label className="text-sm">Ajuste largura (mm)</Label>
                <Input
                  type="number"
                  value={folgas.vidro_largura_offset}
                  onChange={e => onFolgaChange('vidro_largura_offset', Number(e.target.value))}
                />
              </div>
              <div className="space-y-2">
                <Label className="text-sm">Ajuste altura (mm)</Label>
                <Input
                  type="number"
                  value={folgas.vidro_altura_offset}
                  onChange={e => onFolgaChange('vidro_altura_offset', Number(e.target.value))}
                />
              </div>
            </div>
          </div>
