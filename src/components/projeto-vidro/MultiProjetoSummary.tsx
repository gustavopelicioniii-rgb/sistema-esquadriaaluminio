...
export function MultiProjetoSummary({ projetos, onClose }: MultiProjetoSummaryProps) {
  const totalArea = projetos.reduce((sum, p) => {
    return (
      sum +
      p.itens.reduce(
        (s, it) => s + calcAreaEfetiva(it.larguraMm, it.alturaMm, p.areaMinimaM2) * it.quantidade,
        0
      )
    );
  }, 0);
  const totalValor = projetos.reduce((sum, p) => {
    const area = p.itens.reduce(
      (s, it) => s + calcAreaEfetiva(it.larguraMm, it.alturaMm, p.areaMinimaM2) * it.quantidade,
      0
    );
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <BarChart3 className="h-5 w-5 text-primary" /> Resumo Multi-Projeto
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          ...
          <Table>
            ...
            <TableBody>
              {projetos.map(p => {
                const area = p.itens.reduce(
                  (s, it) =>
                    s + calcAreaEfetiva(it.larguraMm, it.alturaMm, p.areaMinimaM2) * it.quantidade,
                  0
                );
                return (
                  <TableRow key={p.id}>
          ...
          <div className="flex justify-between items-center border-t pt-3 font-bold text-sm">
            <span>
              {totalItens} vidro(s) em {projetos.length} projeto(s)
            </span>
