...
import { Plus, Search, Pencil, ArrowDownUp, TrendingUp, TrendingDown, History, Package, AlertTriangle, Loader2 } from "lucide-react";
...
const Estoque = () => {
  ...
  const [movDialog, setMovDialog] = useState<{ item: EstoqueItem; tipo: "entrada" | "saida" | "ajuste" } | null>(null);
  const [movQtd, setMovQtd] = useState(0);
  const [movObs, setMovObs] = useState("");
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState(emptyForm);
  const [movimentoDialogOpen, setMovimentoDialogOpen] = useState(false);
  const [movimentoData, setMovimentoData] = useState<{ produto_id: string; tipo_movimento: string; quantidade: number; observacao: string }>({
  ...
  const getStockStatus = (quantidade: number, minimo: number) => {
    if (quantidade <= 0) return { label: "Sem estoque", color: "bg-destructive", icon: <AlertTriangle className="h-3 w-3" /> };
  ...
  const handleMov = async () => {
    ...
    toast.success(`${tipo === "entrada" ? "Entrada" : tipo === "saida" ? "Saída" : "Ajuste"} registrada: ${movQtd} ${movDialog.item.unidade}`);
    setMovDialog(null);
    setMovQtd(0);
    setMovObs("");
    fetchAll();
    ...
    <div className="space-y-4 sm:space-y-6 pb-20 sm:pb-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" className="gap-2" onClick={() => setMovimentoDialogOpen(true)}>
            <ArrowDownUp className="h-4 w-4" /> Movimentar
          </Button>
          <Button size="sm" className="gap-2" onClick={openNew}><Plus className="h-4 w-4" /> Novo Item</Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { label: "Sem Estoque", value: semEstoque, icon: AlertTriangle, color: "text-red-600 bg-red-50" },
          { label: "Baixo", value: estoqueBaixo, icon: AlertTriangle, color: "text-yellow-600 bg-yellow-50" },
          { label: "OK", value: bemAbastecido, icon: TrendingUp, color: "text-green-600 bg-green-50" },
        ].map(({ label, value, icon: Icon, color }) => (
          <div key={label} className="flex items-center gap-2.5 rounded-lg border bg-card p-3">
            <div className={cn("h-9 w-9 rounded-full flex items-center justify-center shrink-0", color.split(" ")[1])}>
        ...
        <TabsContent value="inventario" className="space-y-4">
          ...
          <div className="rounded-lg border bg-card shadow-sm overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {loading ? (
                  <TableRow><TableCell colSpan={7} className="h-24 text-center text-muted-foreground text-xs sm:text-sm">
                  ...
                  return (
                    <TableRow key={item.id}>
                      ...
                      <TableCell className={cn("text-xs sm:text-sm", item.quantidade <= item.minimo ? "text-destructive font-bold" : "font-medium")}>
                      ...
                      <TableCell>
                        <span className={cn("inline-flex items-center gap-1 text-[10px] sm:text-xs font-semibold", item.quantidade <= 0 ? "text-destructive" : item.quantidade < item.minimo ? "text-yellow-600" : "text-green-600")}>
                          <span className={cn("h-1.5 w-1.5 sm:h-2 sm:w-2 rounded-full", item.quantidade <= 0 ? "bg-destructive" : item.quantidade < item.minimo ? "bg-yellow-500" : "bg-green-500")} />
                          {status.label}
                        </span>
                      </TableCell>
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-0.5">
                          <Button variant="ghost" size="icon" className="h-7 w-7 sm:h-8 sm:w-8" onClick={() => { setMovDialog({ item, tipo: "entrada" }); setMovQtd(0); setMovObs(""); }}>
            ...
            <CardContent className="pt-4">
              <div className="space-y-3">
                <div className="space-y-1.5">
                  <Label>Produto</Label>
                  <Select value={movimentoData.produto_id} onValueChange={v => setMovimentoData(d => ({ ...d, produto_id: v }))}>
                    <SelectTrigger><SelectValue placeholder="Selecione..." /></SelectTrigger>
                    <SelectContent>{itens.map(i => <SelectItem key={i.id} value={i.id}>{i.codigo} – {i.produto}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div className="space-y-1.5">
                  <Label>Tipo</Label>
                  <div className="flex gap-2">
                    {(["entrada", "saida", "ajuste"] as const).map(t => (
                      <Button key={t} variant={movimentoData.tipo_movimento === t ? "default" : "outline"} className="flex-1"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <Input type="number" min={0} value={movimentoData.quantidade || ""} onChange={e => setMovimentoData(d => ({ ...d, quantidade: Number(e.target.value) }))} />
                  </div>
                  <div className="space-y-1.5">
                    <Label>Observação</Label>
                    <Input placeholder="Ex: Compra reposição" value={movimentoData.observacao} onChange={e => setMovimentoData(d => ({ ...d, observacao: e.target.value }))} />
                  </div>
                </div>
                <Button className="w-full" onClick={handleMovimento} disabled={!movimentoData.produto_id || !movimentoData.tipo_movimento || !movimentoData.quantidade}>
          ...
          {movimentos.length === 0 ? (
            ...
            <div className="space-y-2">
              {movimentos.map((mov) => {
                ...
                return (
                  <div key={mov.id} className="flex items-center justify-between p-3 rounded-lg border bg-card">
                    <div className="flex items-center gap-3">
                      <div className={cn("h-9 w-9 rounded-full flex items-center justify-center shrink-0",
                        ...
                        {isEntrada ? <TrendingUp className={cn("h-4 w-4 text-green-600")} /> :
                         isAjuste ? <ArrowDownUp className={cn("h-4 w-4 text-blue-600")} /> :
                         <TrendingDown className={cn("h-4 w-4 text-red-600")} />}
                      </div>
                      <div>
                        <p className="text-sm font-medium">{item?.produto || "Produto removido"}</p>
                        <p className="text-xs text-muted-foreground">
                          {isEntrada ? "Entrada" : isAjuste ? "Ajuste" : "Saída"}: <span className="font-semibold">{mov.quantidade} {item?.unidade}</span>
                          {mov.observacao ? ` — ${mov.observacao}` : ""}
                        </p>
                      </div>
                    </div>
                    <p className="text-xs text-muted-foreground whitespace-nowrap ml-4">{formatDate(mov.created_at)}</p>
                  </div>
                );
              })}
            </div>
      ...
      <Dialog open={!!movDialog} onOpenChange={() => setMovDialog(null)}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            <div className="flex gap-2">
              {(["entrada", "saida", "ajuste"] as const).map(t => (
                <Button key={t} variant={movDialog?.tipo === t ? "default" : "outline"} className="flex-1" size="sm"
            ...
            <div className="space-y-1.5">
              <Label>Observação</Label>
              <Input placeholder="Opcional" value={movObs} onChange={(e) => setMovObs(e.target.value)} />
            </div>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                <Label>Observação</Label>
                <Input placeholder="Opcional" value={movimentoData.observacao} onChange={e => setMovimentoData(d => ({ ...d, observacao: e.target.value }))} />
              </div>
            </div>
            <div className="space-y-1.5">
              <Label>Tipo</Label>
              <div className="flex gap-2">
                {(["entrada", "saida", "ajuste"] as const).map(t => (
                  <Button key={t} variant={movimentoData.tipo_movimento === t ? "default" : "outline"} className="flex-1" size="sm"
                    onClick={() => setMovimentoData(d => ({ ...d, tipo_movimento: t }))}>
                    {t === "entrada" ? "Entrada" : t === "saida" ? "Saída" : "Ajuste"}
                  </Button>
                ))}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleMovimento} disabled={!movimentoData.produto_id || !movimentoData.tipo_movimento || !movimentoData.quantidade}>Registrar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
