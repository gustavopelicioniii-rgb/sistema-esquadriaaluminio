...
import { SummaryCards, CutsTable, GlassTable, ComponentsTable, BarsVisualization, SummaryTable } from "@/components/calculo";
...
export default function CalculoEsquadrias() {
  ...
  const [pdfPreview, setPdfPreview] = useState({ open: false, title: "", blobUrl: null as string | null, filename: "", loading: false });
  ...
  const handleCalculate = async () => {
    if (!selectedTypology || !width || !height) { toast.error("Preencha todos os campos obrigatórios"); return; }
    const W = parseFloat(width); const H = parseFloat(height);
    if (W <= 0 || H <= 0) { toast.error("As dimensões devem ser maiores que zero"); return; }
    if (!typology) { toast.error("Tipologia não encontrada"); return; }

    setCalculating(true);
    try {
      const [cutRules, glassRules, components] = await Promise.all([
        getEffectiveCutRules(selectedTypology, typology._isCustom ?? false, typology._baseTypologyId),
        getEffectiveGlassRules(selectedTypology, typology._isCustom ?? false, typology._baseTypologyId),
        getEffectiveComponents(selectedTypology, typology._isCustom ?? false, typology._baseTypologyId),
      ]);
      const output = calculateTypology(
        { typology_id: selectedTypology, width_mm: W, height_mm: H, quantity: parseInt(quantity) || 1 },
        cutRules, glassRules, components, typology.name, typology.num_folhas, typology
      );
      setResult(output);
  ...
  const handleReset = () => {
    setResult(null); setBarResults([]); setWidth(""); setHeight("");
    setQuantity("1"); setSelectedTypology("");
  };
  ...
  const handlePdf = async (type: "barras" | "padroes" | "impressao") => {
    ...
    const titles = { barras: "Relação de Barras", padroes: "Padrões de Cortes", impressao: "Impressão da Obra" };
    ...
    } catch { setPdfPreview(p => ({ ...p, loading: false })); }
  };
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-1">Motor de cálculo de cortes, vidros e componentes</p>
        </div>
        {result && (
      ...
      {/* Input Form */}
      <Card>
        <CardHeader><CardTitle className="text-base flex items-center gap-2"><Ruler className="h-4 w-4 text-primary" /> Dados do Vão</CardTitle></CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              ...
              <Popover open={fabricanteOpen} onOpenChange={setFabricanteOpen}>
                ...
                <PopoverContent className="w-[240px] p-0" align="start">
                  <Command>
                    ...
                    <CommandList><CommandEmpty>Nenhum fabricante encontrado.</CommandEmpty>
                      <CommandGroup>
                        {manufacturers.filter(m => m.active).map(m => {
                          ...
                          return (
                            <CommandItem key={m.id} value={m.name} onSelect={() => {
                              ...
                              <Check className={cn("mr-2 h-4 w-4", isSelected ? "opacity-100" : "opacity-0")} />{m.name}
                            </CommandItem>
                          );
                        })}
                      </CommandGroup>
            ...
            <div className="space-y-2">
              ...
              <Select value={selectedLine} onValueChange={v => { setSelectedLine(v); setSelectedTypology(""); setResult(null); setSelectedColor("natural"); }}>
                ...
                <SelectContent>
                  {productLines.filter(l => l.manufacturer_id === productLines.find(pl => pl.id === selectedLine)?.manufacturer_id)
                    .map(l => <SelectItem key={l.id} value={l.id}>{l.name} ({l.bitola_mm}mm)</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={selectedTypology} onValueChange={setSelectedTypology}>
                ...
                <SelectContent>{filteredTypologies.map(t => <SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-2">
              <TooltipProvider><Tooltip>
                <TooltipTrigger asChild><Label className="cursor-help">Largura (mm) ⓘ</Label></TooltipTrigger>
                <TooltipContent><p>Mín: {widthLimits.min}mm — Máx: {widthLimits.max}mm</p></TooltipContent>
              </Tooltip></TooltipProvider>
              <Input type="number" placeholder={`${widthLimits.min} – ${widthLimits.max}`} min={widthLimits.min} max={widthLimits.max} value={width} onChange={e => setWidth(e.target.value)} />
            ...
            <div className="space-y-2">
              <TooltipProvider><Tooltip>
                <TooltipTrigger asChild><Label className="cursor-help">Altura (mm) ⓘ</Label></TooltipTrigger>
                <TooltipContent><p>Mín: {heightLimits.min}mm — Máx: {heightLimits.max}mm</p></TooltipContent>
              </Tooltip></TooltipProvider>
              <Input type="number" placeholder={`${heightLimits.min} – ${heightLimits.max}`} min={heightLimits.min} max={heightLimits.max} value={height} onChange={e => setHeight(e.target.value)} />
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={handleCalculate} className="flex-1 gap-2" disabled={calculating}>
                  <Calculator className="h-4 w-4" /><span className="hidden sm:inline">Calcular</span><span className="sm:hidden">Calc</span>
                </Button>
              </div>
            </div>
          </div>
      ...
      {selectedTypology && (
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2"><Eye className="h-4 w-4 text-primary" /> Pré-visualização</CardTitle></CardHeader>
          <CardContent>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {["natural", "branco", "preto", "bronze", "prateado"].map(colorId => (
                      <button key={colorId} onClick={() => setSelectedColor(colorId)}
                        className={cn("w-8 h-8 rounded-full border-2 transition-all", selectedColor === colorId ? "border-primary scale-110" : "border-transparent opacity-60 hover:opacity-100")}
                        style={{ backgroundColor: getColorById(colorId).hex }} title={getColorById(colorId).name} />
                    ))}
                  </div>
                </div>
                <p className="text-xs text-muted-foreground">{selectedTyp?.name}{width && height ? ` — ${width} × ${height} mm` : ""}</p>
              </div>
                    ...
                    profiles={(() => {
                      ...
                      return result.cuts
                        ...
                        .map(c => ({ code: c.profile_code, name: c.piece_name, type: c.piece_function,
                          weight_per_meter: c.weight_kg > 0 && c.cut_length_mm > 0 ? (c.weight_kg / c.quantity) / (c.cut_length_mm / 1000) : undefined }));
                    })()}
        ...
        <>
          ...
          <Card>
            <Tabs defaultValue="cuts">
              <CardHeader className="pb-0 px-3 sm:px-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                    <Button variant="outline" size="sm" className="gap-1.5 flex-1 sm:flex-none" onClick={() => handlePdf("barras")}>
                      <Eye className="h-4 w-4" /> Rel. Barras
                    </Button>
                    <Button variant="outline" size="sm" className="gap-1.5 flex-1 sm:flex-none" onClick={() => handlePdf("padroes")}>
                      <Eye className="h-4 w-4" /> Padrões Corte
                    </Button>
                    <Button variant="outline" size="sm" className="gap-1.5 flex-1 sm:flex-none" onClick={() => handlePdf("impressao")}>
              ...
              <TabsContent value="cuts"><CardContent className="px-3 sm:px-6"><CutsTable cuts={result.cuts} /></CardContent></TabsContent>
              <TabsContent value="glass"><CardContent className="px-3 sm:px-6"><GlassTable glasses={result.glasses} /></CardContent></TabsContent>
              <TabsContent value="components"><CardContent className="px-3 sm:px-6"><ComponentsTable components={result.components} /></CardContent></TabsContent>
              <TabsContent value="bars"><CardContent className="px-3 sm:px-6"><BarsVisualization barResults={barResults} /></CardContent></TabsContent>
              <TabsContent value="summary"><CardContent className="px-3 sm:px-6"><SummaryTable profilesSummary={result.profiles_summary} totalAluminumWeight={result.total_aluminum_weight_kg} /></CardContent></TabsContent>
            </Tabs>
          </Card>
        </>
      )}
    ...
    <PdfPreviewDialog
      ...
      onDownload={() => {
        if (pdfPreview.blobUrl) {
          ...
          a.href = pdfPreview.blobUrl; a.download = pdfPreview.filename; a.click();
        }
      }}
    />
    </>
