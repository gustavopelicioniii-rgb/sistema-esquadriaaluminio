...
const Producao = () => {
  ...
  useEffect(() => {
    if (!authLoading && user) fetchPedidos();
  }, [fetchPedidos, authLoading, user]);
  ...
  const filtered = pedidos.filter(op => {
    ...
    if (search) {
      ...
      if (!op.cliente.toLowerCase().includes(s) && !op.pedido_num.toString().includes(s))
        return false;
    }
    // Period filter
  ...
  const handleConcluir = async (op: Pedido) => {
    const { error } = await supabase
      .from('pedidos')
      .update({ status: 'concluido', etapa: 'Finalizado' } as any)
      .eq('id', op.id);
    if (error) {
  ...
  const handleCancelar = async () => {
    ...
    toast.error('Pedido cancelado', {
      description: `Pedido ${cancelConfirm.pedido_num} foi removido.`,
    });
    ...
    <PullToRefresh onRefresh={fetchPedidos}>
      <div className="space-y-4 sm:space-y-6">
            ...
            <ExportButtons
              getConfig={() => ({
                ...
                summaryCards: [
                  ...
                  {
                    label: 'Valor Total',
                    value: formatCurrency(filtered.reduce((s, o) => s + o.valor, 0)),
                  },
                ],
                rows: filtered.map(o => [
                  String(o.pedido_num),
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            <div className="flex gap-2 overflow-x-auto md:hidden scrollbar-none">
              <Select value={periodFilter} onValueChange={setPeriodFilter}>
                <SelectTrigger className="h-7 text-[10px] w-auto min-w-[120px] shrink-0">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {periodOptions.map(o => (
        ...
        {selectedPedido && (
          <>
            <ReagendarDialog
              open={activeDialog === 'reagendar'}
              onOpenChange={v => !v && closeDialog()}
              pedido={selectedPedido}
            />
            <PagamentosDialog
              open={activeDialog === 'pagamentos'}
              onOpenChange={v => !v && closeDialog()}
              pedido={selectedPedido}
            />
            <ContratoDialog
              open={activeDialog === 'contrato'}
              onOpenChange={v => !v && closeDialog()}
              pedido={selectedPedido}
            />
            <ImpressoesDialog
              open={activeDialog === 'impressoes'}
              onOpenChange={v => !v && closeDialog()}
              pedido={selectedPedido}
            />
            <AlterarEtapaDialog
              open={activeDialog === 'etapa'}
              onOpenChange={v => !v && closeDialog()}
              pedido={selectedPedido}
            />
            <EditarServicoDialog
              open={activeDialog === 'editar'}
              onOpenChange={v => !v && closeDialog()}
              pedido={selectedPedido}
            />
          </>
        ...
        <AlertDialog open={!!cancelConfirm} onOpenChange={v => !v && setCancelConfirm(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Esta ação é irreversível. O pedido e todos os pagamentos associados serão removidos
                permanentemente.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
