...
function findBestMatch(headers: string[], targetKey: string): string | null {
  ...
  for (const syn of synonyms) {
    const match = normalizedHeaders.find(
      h => h.normalized.includes(syn) || syn.includes(h.normalized)
    );
    if (match) return match.original;
  }
    ...
    (f: File) => {
      ...
      Papa.parse(f, {
        ...
        complete: res => {
          ...
          const targetCols =
            mode === 'estoque' ? Object.keys(ESTOQUE_COLUMNS_MAP) : Object.keys(PERFIL_COLUMNS_MAP);
          const autoMap: Record<string, string> = {};
          const usedHeaders = new Set<string>();
          for (const target of targetCols) {
            const match = findBestMatch(
              hdrs.filter(h => !usedHeaders.has(h)),
              target
            );
            if (match) {
    ...
    try {
      if (mode === 'estoque') {
        for (let i = 0; i < rows.length; i++) {
          ...
          if (!codigo || !produto) {
            res.errors.push(`Linha ${i + 2}: código ou produto vazio`);
            continue;
          }
          ...
          const categoria = columnMap.categoria
            ? row[columnMap.categoria]?.trim() || 'Outros'
            : 'Outros';
          ...
          if (existing) {
            const { error } = await supabase
              .from('estoque')
              .update({ produto, quantidade, unidade, categoria, minimo })
              .eq('id', existing.id);
            if (error) {
              res.errors.push(`Linha ${i + 2}: ${error.message}`);
              continue;
            }
            res.updated++;
          } else {
            const { error } = await supabase
              .from('estoque')
              .insert({ codigo, produto, quantidade, unidade, categoria, minimo });
            if (error) {
              res.errors.push(`Linha ${i + 2}: ${error.message}`);
              continue;
            }
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          if (!codigo || !nome) {
            res.errors.push(`Linha ${i + 2}: código ou nome vazio`);
            continue;
          }
          ...
          if (existing) {
            const { error } = await supabase
              .from('estoque')
              .update({ produto, categoria: 'Perfil' })
              .eq('id', existing.id);
            if (error) {
              res.errors.push(`Linha ${i + 2}: ${error.message}`);
              continue;
            ...
            const { error } = await supabase
              .from('estoque')
              .insert({ codigo, produto, categoria: 'Perfil', quantidade: 0, unidade: 'br' });
            if (error) {
              res.errors.push(`Linha ${i + 2}: ${error.message}`);
              continue;
            }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Importe dados de fornecedores para sincronizar com o sistema
          </p>
        </div>
        <Select
          value={mode}
          ...
          <CardContent className="p-8">
            ...
            <div className="text-sm text-muted-foreground space-y-4">
              <p className="font-medium text-foreground mb-2">
                Colunas esperadas ({mode === 'estoque' ? 'Estoque' : 'Perfis'}):
              </p>
              <div className="flex flex-wrap gap-2">
                {Object.entries(colsMap).map(([key, label]) => (
                  <Badge
                    key={key}
                    variant={requiredCols.includes(key) ? 'default' : 'outline'}
                    className="text-xs"
                  >
              ...
              <div className="bg-muted/50 rounded-lg p-4 space-y-3">
                ...
                <ul className="list-disc list-inside space-y-1 text-xs">
                  <li>
                    O arquivo deve ser <strong>.csv</strong> separado por vírgula ou ponto-e-vírgula
                  </li>
                  <li>A primeira linha deve conter os nomes das colunas (cabeçalho)</li>
                  <li>
                    Codificação recomendada: <strong>UTF-8</strong>
                  </li>
                ...
                <p className="font-medium text-foreground text-xs mt-3">
                  Exemplo de CSV — {mode === 'estoque' ? 'Estoque' : 'Perfis'}:
                </p>
              ...
              {Object.entries(colsMap).map(([key, label]) => (
                <div key={key} className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={v =>
                      setColumnMap(prev => ({ ...prev, [key]: v === '__none__' ? '' : v }))
                    }
                  >
