...
function PlanoDetalhe({
  ...
  plano: PlanoCorteType;
  onBack: () => void;
  onUpdate: (id: string, u: Partial<PlanoCorteType>) => Promise<boolean>;
  allTypologies: ExtendedTypology[];
  ...
  const originalCutRules = useMemo(
    () => getCutRulesForTypology(plano.typology_id, baseId),
    [plano.typology_id, baseId]
  );
  const originalGlassRules = useMemo(
    () => getGlassRulesForTypology(plano.typology_id, baseId),
    [plano.typology_id, baseId]
  );

  const defaultCutFolgas = useMemo(() => {
    const map: Record<string, number> = {};
    originalCutRules.forEach(r => {
      map[r.id] = r.constant_mm;
    });
    return map;
  }, [originalCutRules]);
  ...
  const defaultGlassFolgas = useMemo(() => {
    ...
    originalGlassRules.forEach(r => {
      map[r.id] = { w: r.width_constant_mm, h: r.height_constant_mm };
    });
    return map;
  }, [originalGlassRules]);

  const [cutFolgas, setCutFolgas] = useState<Record<string, number>>(defaultCutFolgas);
  const [glassFolgas, setGlassFolgas] =
    useState<Record<string, { w: number; h: number }>>(defaultGlassFolgas);
  const [folgasSaving, setFolgasSaving] = useState(false);
  const [folgasSource, setFolgasSource] = useState<'catalogo' | 'global' | 'personalizada'>(
    'catalogo'
  );
  ...
  useEffect(() => {
    const loadFolgas = async () => {
      const { data: perTyp } = await supabase
        .from('configuracoes')
        .select('valor')
        .eq('chave', folgasKey)
        .maybeSingle();
        ...
        try {
          ...
          if (pOffset !== 0) {
            setCutFolgas(() => {
              const map: Record<string, number> = {};
              originalCutRules.forEach(r => {
                map[r.id] = r.constant_mm + pOffset;
              });
              return map;
            });
          }
          if (vwOffset !== 0 || vhOffset !== 0) {
            setGlassFolgas(() => {
              ...
              originalGlassRules.forEach(r => {
                map[r.id] = {
                  w: r.width_constant_mm + vwOffset,
                  h: r.height_constant_mm + vhOffset,
                };
              });
              return map;
            });
  ...
  const handleExportPDF = useCallback(async () => {
    ...
    setPdfPreview({
      open: true,
      title: 'Relação de Barras',
      blobUrl: null,
      filename: '',
      loading: true,
    });
    try {
      const res = await generateCutListPDF(result, barResults, 'frame-preview-svg', undefined, {
        preview: true,
      });
      if (res) {
        ...
        <Card>
          <CardContent className="p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row items-start gap-4 sm:gap-6">
              <div
                id="frame-preview-svg"
                className="bg-muted/30 rounded-xl p-3 flex items-center justify-center"
              >
                {typ && (
                  <FramePreview
                    width_mm={largura}
          ...
          <Collapsible open={folgasOpen} onOpenChange={setFolgasOpen}>
            <CollapsibleTrigger asChild>
              <button className="w-full flex items-center justify-between px-4 sm:px-6 py-4 hover:bg-muted/30 transition-colors">
                ...
                <ChevronDown
                  className={`h-4 w-4 text-muted-foreground transition-transform ${folgasOpen ? 'rotate-180' : ''}`}
                />
              </button>
            </CollapsibleTrigger>
            <CollapsibleContent>
              <CardContent className="pt-0 pb-4 px-4 sm:px-6 space-y-4">
                <p className="text-xs text-muted-foreground">
                  Ajuste as folgas de cada perfil e vidro. Valores em mm.
                </p>
                      ...
                      {originalCutRules.map(rule => (
                        ...
                        >
                          <div className="shrink-0 w-7 h-7 rounded bg-muted/50 flex items-center justify-center text-primary">
                            <ProfileCrossSection
                              profileType={rule.piece_function || rule.piece_name}
                              profileCode={rule.profile_code ?? ''}
                              size={24}
                            />
                          </div>
                          ...
                          <div className="flex items-center gap-1">
                            <div className="text-center">
                              ...
                              <Input
                                ...
                                onChange={e =>
                                  updateGlassFolga(rule.id, 'w', Number(e.target.value))
                                }
                              />
                            </div>
                            <div className="text-center">
                              ...
                              <Input
                                ...
                                onChange={e =>
                                  updateGlassFolga(rule.id, 'h', Number(e.target.value))
                                }
                              />
                            </div>
                          </div>
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filtered.map(plano => {
          ...
          return (
            <Card
              key={plano.id}
              className="group cursor-pointer hover:shadow-lg hover:border-primary/40 transition-all relative overflow-hidden"
            ...
            >
              ...
              <CardContent className="p-3 space-y-1">
                ...
                <p className="text-xs text-muted-foreground font-mono">
                  L: {plano.largura} × A: {plano.altura} &bull; Qtd: {plano.quantidade}
                </p>
              </CardContent>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        Esta ação não pode ser desfeita. O plano "{plano.nome}" será excluído
                        permanentemente.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
      ...
      {filtered.length === 0 && !loading && (
        <div className="text-center py-12 text-muted-foreground text-sm">
          {planos.length === 0
            ? 'Nenhum plano de corte criado. Clique em Adicionar para começar.'
          ...
          <div className="space-y-4 py-2">
                  ...
                  {allTypologies
                    ...
                    .map(t => (
                      <SelectItem key={t.id} value={t.id}>
                        {t.name}
                        {(t as ExtendedTypology)._isCustom ? ' ★' : ''}
                      </SelectItem>
                    ))}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Ex: Janela 2 Folhas Sala"
              />
            </div>
            <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-2">
                ...
                <Input
                  type="number"
                  value={novoLargura}
                  onChange={e => setNovoLargura(Number(e.target.value))}
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  type="number"
                  value={novoAltura}
                  onChange={e => setNovoAltura(Number(e.target.value))}
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={e => setNovoQuantidade(Math.max(1, Number(e.target.value)))}
                />
              </div>
            </div>
          </div>
