...
export function calcularCustoItem(larguraCm: number, alturaCm: number, quantidade: number, precoM2: number): number {
...
export function calcularTotalCusto(
  ...
  return items.reduce((sum, item) => sum + calcularCustoItem(item.larguraCm, item.alturaCm, item.quantidade, item.precoM2), 0);
}
